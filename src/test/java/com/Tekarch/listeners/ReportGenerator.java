...
public class ReportGenerator implements ITestListener {
    ...
    private static String EXTENT_REPORT_FILE = PROJECT_HOME + File.separator + "Reports" + File.separator
    ...
    public ExtentTest moduleWisetestLogger;
    Date startTime, endTime;
    private String EXTENT_REPORT_PATH = PROJECT_HOME + File.separator + "Reports" + File.separator;
    ...
    public void onTestFailure(ITestResult result) {
        try {
            ...
            testLogger.log(Status.FAIL, "<b><i> Failure Message :: </b></i>" + result.getThrowable().getMessage());
            moduleWisetestLogger.log(Status.FAIL,
                    "<b><i> Failure Message :: </b></i>" + result.getThrowable().getMessage());
            testLogger.log(Status.FAIL, "<div><b><i> Stacktrace :: </b></i></div><div><textarea>"
                    + result.getThrowable().getStackTrace() + "</textarea></div>");
            moduleWisetestLogger.log(Status.FAIL, "<div><b><i> Stacktrace :: </b></i></div><div><textarea>"
                    + result.getThrowable().getStackTrace() + "</textarea></div>");
            testLogger.log(Status.INFO, "Total time taken for test case to execute :: "
                    + ((endTime.getTime() - startTime.getTime()) / 1000) + " Seconds");
            moduleWisetestLogger.log(Status.INFO, "Total time taken for test case to execute :: "
                    + ((endTime.getTime() - startTime.getTime()) / 1000) + " Seconds");
    ...
    public void onTestStart(ITestResult result) {
        ...
        testLogger = extent.createTest(testName + methodParameters);
        testLogger.assignCategory(suiteName);
        testLogger.log(Status.INFO, "<b><i>Test Case Name :: </b></i>\"" + testName + "\"");
        testLogger.log(Status.INFO,
                "<b><i>Description of the test :: </b></i> \"" + result.getMethod().getDescription() + "\"");
        moduleWisetestLogger = moduleWiseExtent.createTest(testName + methodParameters);
        moduleWisetestLogger.assignCategory(suiteName);
        moduleWisetestLogger.log(Status.INFO, "<b><i>Test Case Name :: </b></i>\"" + testName + "\"");
        moduleWisetestLogger.log(Status.INFO,
                "<b><i>Description of the test :: </b></i> \"" + result.getMethod().getDescription() + "\"");
        startTime = new Date();
        filename = testName;
    }
    ...
    public void onTestSuccess(ITestResult result) {
        ...
        testLogger.log(Status.INFO, "Total time taken for test case to execute :: "
                + ((endTime.getTime() - startTime.getTime()) / 1000) + " Seconds");
        testLogger.log(Status.PASS, testName + methodParameters + " test has passed");
        moduleWisetestLogger.log(Status.INFO, "Total time taken for test case to execute :: "
                + ((endTime.getTime() - startTime.getTime()) / 1000) + " Seconds");
        moduleWisetestLogger.log(Status.PASS, testName + methodParameters + " test has passed");
        extent.flush();
        moduleWiseExtent.flush();
        didOnTestStartGotExecuted = false;
    }
    ...
    public void onTestSkipped(ITestResult result) {
        ...
        testLogger.log(Status.INFO,
                "<b><i>Description of the test :: </b></i>\"" + result.getMethod().getDescription() + "\"");
        testLogger.log(Status.SKIP, testName + methodParameters + " test skipped due to :: "
                + result.getThrowable().getStackTrace());
        moduleWisetestLogger.log(Status.INFO,
                "<b><i>Description of the test :: </b></i>\"" + result.getMethod().getDescription() + "\"");
        moduleWisetestLogger.log(Status.SKIP, "<b><i>" + "Test skipped due to :: </b></i><textarea>"
